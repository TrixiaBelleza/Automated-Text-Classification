...
param_grid = {'clf__estimator__C': np.arange(1,10), 
                'clf__estimator__tol': [1, 0.01, 0.001, 0.0001, 0.00000001]}
nb_param_grid = {'clf__estimator__alpha': np.arange(0,10),
                'fit_prior': [True, False]}

# for param in SVC_pipeline.get_params().keys():
#     print(param)
for category in categories:
    
    print('... Processing {}'.format(category))
    clf_cv = GridSearchCV(SVC_pipeline, param_grid, cv=10)
    clf_cv.fit(x_train, train[category])
    print("SVC:")
    print("Tuned Parameters: {}".format(clf_cv.best_params_))
    print("Best score is: {}".format(clf_cv.best_score_))

    # clf_cv = GridSearchCV(LogReg_pipeline, param_grid, cv=10)
    # clf_cv.fit(x_train, train[category])
    # print("LogReg:")
    # print("Tuned Parameters: {}".format(clf_cv.best_params_))
    # print("Best score is: {}".format(clf_cv.best_score_))

    # clf_cv = GridSearchCV(NB_pipeline, param_grid, cv=10)
    # clf_cv.fit(x_train, train[category])
    # print("NaiveBayes:")
    # print("Tuned Parameters: {}".format(clf_cv.best_params_))
    # print("Best score is: {}".format(clf_cv.best_score_))
